...
# ===========================================================================
# Custom WebEnginePage con manejo de nuevas ventanas
# ===========================================================================
class CustomWebEnginePage(QWebEnginePage):
    """
    QWebEnginePage personalizada que maneja peticiones de nuevas ventanas.
    Necesario para que Google Drive/Docs pueda abrir documentos en nuevas pestañas.
    """

    new_window_requested = pyqtSignal(QUrl)  # Señal para notificar nueva ventana
    ...
    def createWindow(self, window_type):
        """
        Sobrescribe createWindow para manejar peticiones de nuevas ventanas.
        Google Drive usa esto para abrir documentos.

        Args:
            window_type: Tipo de ventana solicitada

        Returns:
            QWebEnginePage: Nueva página para la ventana
        """
        logger.info(f"createWindow llamado con tipo: {window_type}")

        # Crear una nueva página temporal
        new_page = CustomWebEnginePage(self.profile(), self.parent())

        # Cuando la nueva página reciba una URL, emitir señal
        new_page.urlChanged.connect(lambda url: self.new_window_requested.emit(url))
...
class SimpleBrowserWindow(QWidget):
    ...
    def add_new_tab(self, url: str = "https://www.google.com", title: str = "Nueva pestaña"):
        ...
        if self.web_profile:
            page = CustomWebEnginePage(self.web_profile, browser)
            browser.setPage(page)
            # Conectar señal para manejar nuevas ventanas (ej: Google Docs)
            page.new_window_requested.connect(lambda url: self.add_new_tab(url.toString(), "Nueva pestaña"))
            logger.debug("Pestaña creada con perfil persistente y soporte para nuevas ventanas")
        else:
            # Crear página sin perfil persistente pero con soporte para nuevas ventanas
            page = CustomWebEnginePage(None, browser)
            browser.setPage(page)
            page.new_window_requested.connect(lambda url: self.add_new_tab(url.toString(), "Nueva pestaña"))
            logger.debug("Pestaña creada con perfil temporal y soporte para nuevas ventanas")

        # Configurar el navegador
        settings = browser.settings()
        settings.setAttribute(QWebEngineSettings.WebAttribute.JavascriptEnabled, True)
        settings.setAttribute(QWebEngineSettings.WebAttribute.PluginsEnabled, True)  # Habilitar plugins para Google Drive
        settings.setAttribute(QWebEngineSettings.WebAttribute.LocalStorageEnabled, True)
        settings.setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessRemoteUrls, True)  # Permitir acceso remoto
        settings.setAttribute(QWebEngineSettings.WebAttribute.JavascriptCanOpenWindows, True)  # Permitir popups de Google
        settings.setAttribute(QWebEngineSettings.WebAttribute.WebGLEnabled, True)  # Habilitar WebGL
        settings.setAttribute(QWebEngineSettings.WebAttribute.Accelerated2dCanvasEnabled, True)  # Aceleración 2D
        settings.setAttribute(QWebEngineSettings.WebAttribute.AllowRunningInsecureContent, True)  # Permitir contenido mixto
