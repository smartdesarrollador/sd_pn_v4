...
from PyQt6.QtWidgets import (
    ...
    QTreeWidget, QTreeWidgetItem, QWidget, QApplication, QMenu, QMessageBox, QSplitter
)
...
class StructureDashboard(QDialog, TaskbarMinimizableMixin):
    ...
    def init_ui(self):
        ...
        # Splitter for tags sidebar and tree view
        ...
        # Tags Filter Sidebar (left panel)
        self.tags_sidebar = TagsFilterSidebar()
        self.tags_sidebar.setMinimumWidth(150)
        self.tags_sidebar.setMaximumWidth(300)
        self.tags_sidebar.tags_filter_changed.connect(self._on_tags_filter_changed)
        splitter.addWidget(self.tags_sidebar)

        # TreeView (right panel)
        self.tree_widget = self.create_tree_widget()
        splitter.addWidget(self.tree_widget)

        # Set initial sizes: 200px for sidebar, rest for tree
    ...
    def populate_tree(self, structure: dict):
        ...
        for category in categories:
            ...
            for item in category['items']:
                ...
                # Convert tags list to comma-separated string for sidebar filter
    ...
    def reset_filters(self):
        ...
        for btn in self.type_filter_buttons.values():
            btn.setChecked(False)
        # Clear tags filter sidebar
        self.tags_sidebar.clear()
    ...
    def on_search_changed(self, query: str, scope_filters: dict):
        ...
        # Update tags sidebar with visible items
        self.tags_sidebar.update_tags_from_tree(self.tree_widget)
    ...
    def _on_tags_filter_changed(self, selected_tags: list):
        """
        Filter tree items based on selected tags

        Args:
            selected_tags: List of tag names that are currently selected
        """
        logger.info(f"Filtering tree by {len(selected_tags)} selected tags")

        root = self.tree_widget.invisibleRootItem()

        # If no tags selected, hide all items
        if not selected_tags:
            ...
            logger.debug("No tags selected, hiding all items")
            return
        ...
        for cat_idx in range(root.childCount()):
            ...
            for item_idx in range(category_item.childCount()):
                ...
                # Only filter actual items, not categories
                ...
                # Show item if it has at least one selected tag
                has_selected_tag = any(tag in selected_tags for tag in item_tags)
                item_widget.setHidden(not has_selected_tag)

                if has_selected_tag:
                    visible_items_count += 1

            # Hide category if no items are visible
            category_item.setHidden(visible_items_count == 0)

            # Expand category if it has visible items
            if visible_items_count > 0:
                category_item.setExpanded(True)

        logger.debug(f"Tag filtering applied: {selected_tags}")
